...
@Log4j
public class RelatorioOutrasDespesasCusteioService implements Serializable{
	...
	public List<String> buscarAnosComRegistros(Unidade unidade) {
		...
		List<String> anos  = this.outrasDespesasDAO.buscarAnosComRegistroCusteio(unidade);
		return anos;
	}
	
	public List<OutrasDespesasTO> buscarDespesasTO(LocalDate dataInicio, LocalDate dataFim, Unidade unidade){
		
		log.info("entrou no service de relatorio outras despesas custeio");
		
		List<OutrasDespesasDTO> despesasDTO = this.outrasDespesasDAO.buscarDespesasDTOCusteio(dataInicio, dataFim, unidade);
